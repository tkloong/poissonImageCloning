...
class SyncedMemory {
	...
	cudaError_t Sync()
	{
		...
		switch (state_) {
			case CPU_IS_NEW:
				e = cudaMemcpy(gpu_, cpu_, sizeof(T)*n_, cudaMemcpyHostToDevice);
				break;
			case GPU_IS_NEW:
				e = cudaMemcpy(cpu_, gpu_, sizeof(T)*n_, cudaMemcpyDeviceToHost);
				break;
		}
		state_ = SYNCED;
		return e;
...
class MemoryBuffer {
	...
	void Alloc(const size_t n)
	{
		if (n != 0) {
			cpu_ = new T[n];
			memset(cpu_, 0, sizeof(T)*n);
			cudaMalloc(&gpu_, sizeof(T)*n);
			cudaMemset(gpu_, 0, sizeof(T)*n);
		}
	}
public:
	void Free()
